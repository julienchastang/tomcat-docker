...
For local HTTPS testing, generate a 30-day self-signed RSA certificate. The Subject Alternative Name matches the example hostname, `tomcat.example.com`. This certificate is not intended for production.

```sh
openssl req -new -newkey rsa:4096 -sha256 -days 30 -nodes -x509 \
    -subj "/CN=tomcat.example.com" \
    -addext "subjectAltName=DNS:tomcat.example.com" \
...
The examples below use the locally built `tomcat-docker:<version>` image from the Installation section. Replace `<version>` with the tag used for that build; no published Docker Hub tag is assumed. Run from the repository directory.

Before starting Tomcat, ensure `server.xml`, `ssl.crt`, and `ssl.key` are readable by its non-root runtime identity (default UID/GID `1000/1000`). For the private key with mode `600`, its host owner must match the runtime UID; arrange host ownership accordingly. If overriding `TOMCAT_USER_ID`/`TOMCAT_GROUP_ID`, adjust host permissions for the actual runtime identity. The entrypoint does not change ownership or permissions on these configuration mounts. Keep the private key restricted; do not make it world-readable.
...
After Tomcat starts, verify HTTPS using the generated certificate as the trust anchor. `--resolve` directs the example hostname to localhost while preserving hostname verification; `--noproxy` bypasses any configured proxy for this local test:

```sh
curl --noproxy tomcat.example.com --cacert ./ssl.crt \
    --resolve tomcat.example.com:8443:127.0.0.1 \
    https://tomcat.example.com:8443/
```

An HTTP 404 is acceptable if no ROOT webapp is deployed: TLS and certificate verification should still succeed. Do not use `-k` to bypass verification.
